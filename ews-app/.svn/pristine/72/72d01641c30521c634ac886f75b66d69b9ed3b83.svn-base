...
public class EmpSelAction implements BusinessAction {
	public EmpSelAction() {
	}
	...
	private String empChkMb = "emp_mb_";

	// 是否需要显示手机号码
	private boolean bNeedMb = true;

	// 显示职务的最低级别
	private String dutylmt = null;
	...
	public void doExecute(BusinessForm parm1, DataManager dm)
		...
		sf.setEmpmbchk(empChkMb);

		// 非0 即为需要显示
		if ("0".equals(sf.getNeedmb()))
			bNeedMb = false;

		dutylmt = sf.getDutylmt();
	...
	private String getSelSql(String sid) {
		...
		sql += " where s.STATE='1' and s.PARENTDEPT=" + sid;
		if (dutylmt != null || "".equals(dutylmt))
			sql += " and (info_class!=2 or(info_class=2 and duty_class<="
					+ dutylmt + "))";

		sql += " order by DEPTRANK";
	...
	private String getDirKind(String strid, String strname, int intNodeCls,
		...
		String strOut = "";
		String strGif = "", strImgCls = "", sTemp = "", sCheckImg = "";
		String strEmpName = "", strMb = "";
		...
		case 3:
			...
			Iterator<DataObject> itEmp = dm.find(
					"select f_getempname(" + empSid
							+ ") as empname,f_get_emp_mb(" + empSid
							+ ") as empMb from dual", null).iterator();
			while (itEmp.hasNext()) {
				strid = "emp_" + empSid + "_";
				DataObject dob = (DataObject) itEmp.next();
				strEmpName = dob.getString("empname");
				strMb = dob.getString("empMb");
				if (bNeedMb) // 需要显示手机号码
					strname = strEmpName + "/" + strMb;
				else
					// 不需要
					strname = strEmpName;
			...
			sCheckImg += empChkHead + empSid + "' src='" + unchkImg;
			sCheckImg += "' width='16' align='absmiddle' onclick=\"return turnit_leaf(this)\">";
			sCheckImg += "<INPUT TYPE='hidden' id='h_" + empChkHead + empSid
					+ "' value='";
			sCheckImg += strEmpName + "'>";
			sCheckImg += "<INPUT TYPE='hidden' id='h_" + empChkMb + strMb
					+ "' value='";
			sCheckImg += strMb + "'>";
		...
		if (iter.hasNext()) { // 还有子目录，显示+的前导图片
			...
			strOut += "<font onclick=\"return turnit_leaf(this)\" ondblclick=\"leaf_dbl(this,Dir_"
					+ stridx + "_,Tag_";
			strOut += stridx + "_)\">" + strname + "</font><br>";
