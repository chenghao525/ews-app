 ...
 */
public class ReportEfficiencyYear implements AjaxInterface {
    ...
    public void doExecute(HttpServletRequest request, HttpServletResponse response, DataManager dm) throws Exception {
        ...
        json.put("S_CREATE_TIME_ED", Integer.parseInt(json.getString("TYPE")) + 1);

        DataObject dob = new DataObject(json);
        Iterator<DataObject> it = rptUtl.getRptType(dob, dm);

        Iterator<DataObject> it2 = rptUtl.getRptTableYearType(dob, dm);

        JSONArray months = rptUtl.getMonths();
