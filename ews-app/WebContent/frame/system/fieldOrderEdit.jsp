...
<jsp:useBean id="fieldOrderBean" class="com.corp.frame.bean.SaveFieldOrderBean"
 scope="page" />

<%
 fieldOrderBean.populate(request);
 if(request.getParameter("b_sure") != null){
   fieldOrderBean.execute();
   response.sendRedirect("fieldEdit.jsp?tableName="+ fieldOrderBean.getDomainName());
 }
 %>
<table border="0" cellpadding="3" cellspacing="1" class="tablebgW">
    <tr>
    <td class="content">更改字段顺序</td>
  </tr>
</table>
...
<%
  pageContext.setAttribute("fieldListColl",
    MetaData.getFieldListGroupByType(fieldOrderBean.getDomainName()));
%>
<logic:iterate id="tempIterate" name="fieldListColl">
<tr class="tableCenter"><td colspan="4"></td></tr>
<extend:iterateGroup id="element" name="tempIterate" column="4">
